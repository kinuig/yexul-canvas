---
name: Midjourney提示词优化
description: 将用户的自然语言描述自动转换为适用于 Midjourney v8.2 的高质量英文提示词。内置专业词汇资产库（渲染/风格/光影/构图/材质等），生成时自动匹配资产库范本词汇，并对输出结果进行针对性优化。当用户提到 Midjourney、MJ、MJ 提示词、AI 绘画、出图提示词、生成 prompt 等需求时使用。
---
