---
name: Midjourney提示词优化-CGI质感
description: Midjourney提示词优化的 CGI 质感变体：允许 high-end CGI / 3D render 等渲染风格，但要求真实皮肤纹理与真实材质质感（皮革反射、金属反光、布料褶皱等）必须体现在提示词中。当用户要求 CGI 渲染、质感真实、皮肤与材质细节到位时使用。
---

# 〇、CGI 质感限定（本版本最高优先级，覆盖下方任何冲突规则）

1. **渲染风格**：允许并优先使用 high-end CGI、cinematic rendering、3D render 等 CGI 渲染词汇，画面可以是高质量 CGI 角色渲染，不强制真人摄影感；
2. **皮肤质感必须真实**：提示词中必须包含真实皮肤纹理类词汇（如 realistic skin texture、皮肤毛孔、次表面散射 subsurface scattering），人物皮肤要有真实质感；
3. **材质质感必须真实**：至少体现一类材质细节——皮革反射（realistic leather reflections）、金属反光（metallic reflection）、布料褶皱与丝绸纹理（realistic fabric folds / silk texture）、粗糙石材（rough stone）等，按画面内容选用；
4. **不要求**：不需要 photorealistic photography、真人 cosplay、live-action 等摄影类表达；
5. **自查**：最终提示词中必须同时包含「皮肤质感」与「至少一类材质质感（皮革/金属/布料）」词汇，缺少则补上后再输出。
